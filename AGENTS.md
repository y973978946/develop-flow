...
- `skills/` — 5 个 skills，每个在 `skills/<name>/` 中，`skill.md`（或 `SKILL.md`）为入口
- `agents/` — 16 个 Agent 定义（`.md` 文件），6 个核心 + 10 个辅助
...
| `parse-attachment` | `skills/parse-attachment/SKILL.md` | 自动（附件解析） |
...
`skills/develop-flow/learn/` 实现学习闭环：
- **capture**：Phase 6 结束后自动记录信号
- **apply**：Phase 0 开始时自动注入历史经验
- **distill**：累积 5 条信号后提醒，聚类信号→更新知识库
...
`skills/develop-flow/obsidian/` 提供 Obsidian 集成脚本：
- `sync-playbook-to-obsidian.sh` — playbook → Obsidian 同步
- `sync-obsidian-to-playbook.sh` — Obsidian → playbook 反向同步
- `OBSIDIAN-SETUP.md` — 配置指南
...
- `skills/develop-flow/learn/` 定义学习闭环
